...
inline bool bounce(Board *board, uint16_t xTarget, uint16_t yTarget)
{
    return *boardVisitedAt(board, xTarget, yTarget);
}

inline bool directionOccupied(Board* board, uint16_t xStart, uint16_t yStart, Direction direction)
{
...
bool isMovePossible(Board* board, uint16_t xStart, uint16_t yStart,  uint16_t xTarget, uint16_t yTarget)
{
    ...
    if(direction != Error)
        return directionOccupied(board, xStart, yStart, direction);
    else 
        return false;
}